...
use cosmwasm_std::{to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult, QueryRequest};
use cosmwasm_std::WasmQuery::Smart;

use cw2::set_contract_version;
use tefi_oracle::proxy::{ProxyPriceResponse, ProxyQueryMsg};

use crate::msg::{CandleQuery, CandleResponse, ConfigResponse, InstantiateMsg, QueryMsg};
...
pub fn query_price(deps: Deps, _env: Env, symbol: String) -> StdResult<ProxyPriceResponse> {
    ...
    let res: CandleResponse = deps.querier.query(&QueryRequest::Wasm(Smart {
        contract_addr: config.source_addr.to_string(),
        msg: to_binary(&CandleQuery::Price {denom: symbol})?,
    }))?;

    Ok(ProxyPriceResponse {
        rate: res.rate,
        last_updated: res.last_updated_timestamp.u64(),
    })
}
